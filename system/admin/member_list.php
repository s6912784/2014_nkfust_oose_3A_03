...
if($_GET[b]=='del'){
    ...
    echo "<script>location.replace('?a=member_list.php&m_id=$_GET[m_id]')</script>";
}

$rf = mysql_num_rows(mysql_query("select * from m_followlist where m_id='$_GET[m_id]'"));
...
<table align="center" width="700">
    ...
    <?
	$ret = mysql_query("select * from m_followlist where m_id='$_GET[m_id]'");
	while($row = mysql_fetch_array($ret)){
		...
		?>
	<tr align="center">
    	<td><a href="?a=product_show_in.php&p_id=<?=$row[p_id]?>"><img src="product/s<?=$p[p_pic]?>"></a></td>
        <td><?=$p[p_name]?></td>
        <td><?=$s[stype_name]?></td>
        <td><?=$t[t_name]?></td>
        <td>
        ...
        <td><a href="?a=<?=$a?>&f_id=<?=$row[f_id]?>&b=del&m_id=<?=$row[m_id]?>"><img src="../icon/trash.png" onclick="return confirm('您確定要刪除嗎!?')"></a></td>
    </tr>
    <? }?>
</table>
<? 
